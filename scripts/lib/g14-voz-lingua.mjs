...
const MARCAS = {
  ...
  nl: {
    nome: 'neerlandês',
    // Neerlandês e alemão são vizinhos e compartilham muita funcional curta
    // (in, is, op, en, dat). `forte` fica com o que é distintivamente
    // NEERLANDÊS e quase nunca alemão: o dígrafo ⟨ij⟩ (mij, zijn, tijd, wij),
    // as fórmulas de cortesia (alstublieft, dankjewel, alsjeblieft) e algumas
    // funcionais que o alemão não tem na mesma forma (het, een, niet, jij).
    forte: /\bij\b|ij[a-z]|\b(alstublieft|alsjeblieft|dankjewel|dank je|graag gedaan|hoeveel|gezellig|proost|meneer|mevrouw|het|een|niet|jij|jullie|hoor|even)\b/i,
    propria:
      /\b(de|het|een|en|is|zijn|niet|wel|dat|die|dit|deze|ik|je|jij|jullie|hij|zij|wij|met|voor|van|op|aan|te|om|maar|ook|nog|al|even|graag|hoor|dus|want|of|als|dan|waar|wat|wie|hoe|waarom|kan|kun|kunt|wil|wilt|heb|heeft|hebben|ben|bent|naar|bij|over|onder|tegen|zonder|tussen|hier|daar|nu|heel|erg|meer|alles|niets|iets|goed|dank)\b/i
  },
